...
// have async functions on them.  Usage is still similar
// since its  still wrapped in a struct.

pub struct ListPlantsPrompt {
    region_name: String,
    shade: String,
    moisture: String,
}

impl ListPlantsPrompt {
    pub fn new(region_name: &str, shade: &str, moisture: &str) -> Self {
        Self {
    ...
    pub fn build_payload(&self) -> openai::ChatCompletionRequest {
        let text = build_prompt(&self.region_name, &self.shade, &self.moisture);
